 ...
 */
package com.wl4g.paas.uci.utils;
...
import static com.wl4g.paas.uci.utils.HookCommandHolder.CommandType.*;

/**
 * {@link HookCommandHolder}
 * 
 ...
 * @see
 */
public abstract class HookCommandHolder {
	...
	/**
	 * @param message
	 *            like this: $ci deploy —projects=datav,portal
	...
	private static BuildCommand parseBuild(String command) throws ParseException {
		...
		CommandLine l = CommandUtils.newBuilder()
				...
				.option(TEST.getOpt(), TEST.getLongOpt(), TEST.getDefaultValue(), TEST.getDescription()).build(argStr.split(" "));
	...
	private static DeployCommand parseDeploy(String command) throws ParseException {
		...
		CommandLine l = CommandUtils.newBuilder()
				...
				.option(ENV.getOpt(), ENV.getLongOpt(), ENV.getDefaultValue(), ENV.getDescription()).build(argStr.split(" "));
	...
	public static enum CommandType {
		...
		PROJECTS("p", "projects", null, "which projects need deploy"), BRANCH("b", "branch", "", "which branch need deploy"), ENV(
				"e", "env", "", "deploy environment"), TEST("t", "test", "", "is projects need test");
