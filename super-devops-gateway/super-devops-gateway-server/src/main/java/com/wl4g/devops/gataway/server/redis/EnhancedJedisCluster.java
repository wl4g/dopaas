 ...
 */
package com.wl4g.devops.gataway.server.redis;
...
import static com.wl4g.devops.gataway.server.redis.EnhancedJedisClusterCommand.EnhancedJedisClusterConntionHandler;
...
public class EnhancedJedisCluster extends JedisCluster {
	...
	public ScanResult<Entry<byte[], byte[]>> hscan(final byte[] key, final byte[] cursor) {
		checkArgumentsSpecification(key);
		return new EnhancedJedisClusterCommand<ScanResult<Entry<byte[], byte[]>>>(connectionHandler, maxAttempts) {
			@Override
			public ScanResult<Entry<byte[], byte[]>> doExecute(Jedis connection) {
				return connection.hscan(key, cursor);
			}
		}.runBinary(key);
	}
	 ...
	 */
	protected void checkArgumentsSpecification(final byte[]... keys) throws RuntimeException {
	 ...
	 * @throws ArgumentsSpecificationException
	 */
	protected void checkArgumentsSpecification(final String... keys) throws RuntimeException {
		RedisProtocolUtil.checkArgumentsSpecification(asList(keys));
	}
	...
	public static abstract class RedisProtocolUtil {
		 ...
		 * @throws ArgumentsSpecificationException
		 */
		public static void checkArgumentsSpecification(final List<?> keys) throws RuntimeException {
