...
public class MetricAlarmThread extends GenericTaskRunner {
    ...
    public void run() {
        ...
        String instandId = jedisService.get(REDIS_PRO_UMC_IP_TO_ID+instance);
        if(StringUtils.isBlank(instandId)){
            return;
        }
        String json = jedisService.get(REDIS_PRO_UMC_ALARM+String.valueOf(instandId));
        AlarmConfigRedis alarmConfigRedis = JacksonUtils.parseJSON(json, AlarmConfigRedis.class);
