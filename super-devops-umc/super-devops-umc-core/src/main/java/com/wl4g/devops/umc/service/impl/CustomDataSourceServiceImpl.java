...
public class CustomDataSourceServiceImpl implements CustomDataSourceService {
    ...
    public void save(BaseDataSource baseDataSource) {
        ...
        if(baseDataSource instanceof MysqlDataSource){
            MysqlDataSource mysqlDataSource = (MysqlDataSource) baseDataSource;
            if (StringUtils.equalsAnyIgnoreCase(mysqlDataSource.getPassword(), "******") || StringUtils.isBlank(mysqlDataSource.getPassword())) {
    ...
    public CustomDataSource model2Properties(BaseDataSource baseDataSource){
        ...
        try {
            List<CustomDataSourceProperties> customDataSourceProperties = objectToCustomDataSourceProperties(baseDataSource, baseDataSource.getId());
        ...
        if(MYSQL.toString().equalsIgnoreCase(baseDataSource.getProvider()) && baseDataSource instanceof MysqlDataSource){
            MysqlDataSource mysqlDataSource = (MysqlDataSource)baseDataSource;
            //url
            CustomDataSourceProperties url  = new CustomDataSourceProperties();
            url.setDataSourceId(customDataSource.getId());
            url.setKey(URL);
            url.setValue(mysqlDataSource.getUrl());
            customDataSourceProperties.add(url);
            //username
            CustomDataSourceProperties username  = new CustomDataSourceProperties();
            username.setDataSourceId(customDataSource.getId());
            username.setKey(USERNAME);
            username.setValue(mysqlDataSource.getUsername());
            customDataSourceProperties.add(username);
            //password
            CustomDataSourceProperties password  = new CustomDataSourceProperties();
    ...
    public List<CustomDataSourceProperties> objectToCustomDataSourceProperties(Object obj,Integer dataSourceId) throws IllegalAccessException {
