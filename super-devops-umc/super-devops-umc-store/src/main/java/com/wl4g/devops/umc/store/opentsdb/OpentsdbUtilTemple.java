 ...
 */
public class OpentsdbUtilTemple {
    ...
    public void query() throws Exception {
        ...
        Query query = Query.begin("7d-ago")
                ...
                .build();
        // 同步查询
        List<QueryResult> resultList = OpentsdbConf.getClient().query(query);

        // 异步查询
        OpentsdbConf.getClient().query(query, new QueryHttpResponseCallback.QueryCallback() {
            @Override
    ...
    public void queryLast() throws Exception {
        LastPointQuery query = LastPointQuery.sub(LastPointSubQuery.metric("point")
                ...
                .backScan(1000)
                .build();
        List<LastPointQueryResult> lastPointQueryResults = OpentsdbConf.getClient().queryLast(query);
    }
    ...
    public void delete() throws Exception {
        Query query = Query.begin("7d-ago")
                .sub(SubQuery.metric("metric.test")
                        ...
                        .build())
                .build();
        OpentsdbConf.getClient().delete(query);
    }
    ...
    public void querySuggest() throws Exception{
        SuggestQuery query = SuggestQuery.type(SuggestQuery.Type.METRICS)
                .build();
        List<String> suggests = OpentsdbConf.getClient().querySuggest(query);
    }
