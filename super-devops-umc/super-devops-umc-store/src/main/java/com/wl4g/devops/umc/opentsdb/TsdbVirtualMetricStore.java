...
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_BLOCK_IN;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_BLOCK_OUT;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_CPU;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_MEM_PERC;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_MEM_USAGE;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_NET_IN;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.METRIC_DOCKER_NET_OUT;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.TAG_DOCKER_CONTAINER_ID;
import static com.wl4g.devops.common.constants.UMCDevOpsConstants.TAG_DOCKER_NAME;
...
public class TsdbVirtualMetricStore implements VirtualMetricStore {
	...
	public boolean save(Docker docker) {
		...
		for (Docker.DockerInfo dockerInfo : docker.getDockerInfo()) {
			try {
				Point cpuPerc = Point.metric(METRIC_DOCKER_CPU).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, nf.parse(dockerInfo.getCpuPerc())).build();

				Point memUsage = Point.metric(METRIC_DOCKER_MEM_USAGE).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, toB(split(dockerInfo.getMemUsage())[0])).build();

				Point memPerc = Point.metric(METRIC_DOCKER_MEM_PERC).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, nf.parse(dockerInfo.getMemPerc())).build();

				String net[] = split(dockerInfo.getNetIO());
				Point netIn = Point.metric(METRIC_DOCKER_NET_IN).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, toB(net[0])).build();
				Point netOut = Point.metric(METRIC_DOCKER_NET_OUT).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, toB(net[1])).build();

				String block[] = split(dockerInfo.getBlockIO());
				Point blockIn = Point.metric(METRIC_DOCKER_BLOCK_IN).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
						.value(timestamp, toB(block[0])).build();
				Point blockOut = Point.metric(METRIC_DOCKER_BLOCK_OUT).tag(TAG_ID, docker.getPhysicalId())
						.tag(TAG_DOCKER_CONTAINER_ID, dockerInfo.getContainerId()).tag(TAG_DOCKER_NAME, dockerInfo.getName())
