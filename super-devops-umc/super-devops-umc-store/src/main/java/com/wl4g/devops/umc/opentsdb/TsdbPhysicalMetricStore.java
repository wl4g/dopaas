...
public class TsdbPhysicalMetricStore implements PhysicalMetricStore {
	...
	public boolean save(Mem mem) {
		...
		Point usedPercent = Point.metric(METRIC_MEM_USED_PERCENT).tag(TAG_ID, mem.getId())
	...
	public boolean save(Disk disk) {
		...
		for (DiskInfo diskInfo : disk.getDiskInfos()) {
			...
			Point free = Point.metric(METRIC_DISK_FREE).tag(TAG_ID, disk.getId()).tag(TAG_DISK_DEVICE, partitionStat.getDevice())
					.value(timestamp, usage.getFree()).build();
			Point used = Point.metric(METRIC_DISK_USED).tag(TAG_ID, disk.getId()).tag(TAG_DISK_DEVICE, partitionStat.getDevice())
