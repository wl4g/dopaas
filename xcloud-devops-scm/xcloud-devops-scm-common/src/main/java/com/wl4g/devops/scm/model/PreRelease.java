 ...
 */
package com.wl4g.devops.scm.model;
...
public class PreRelease extends GenericInfo {
	...
	public void validation(boolean validVersion, boolean validReleaseId) {
		...
		notEmpty(getInstances(), "`releaseInstances` is not allowed to be null.");
		getInstances().stream().forEach((i) -> {
			notNull(i, "`releaseInstances` is not allowed to be empty.");
			i.validation();
		});
	}
