...
public class ScmClientProperties {
	 ...
	 */
	public static final String PREFIX = "spring.cloud.devops.scm.client.config";
	...
	/**
	 * The default profile to use when fetching remote configuration
	...
	/**
	 * The label name to use to pull remote configuration properties. The
	...
	/**
	 * Flag to indicate that failure to connect to the server is fatal (default
	 * false).
	 */
	...
	private ScmClientProperties() {
	}

	public ScmClientProperties(Environment environment) {
	...
	public ScmClientProperties override(Environment environment) {
		ScmClientProperties override = new ScmClientProperties();
		BeanUtils.copyProperties(this, override);
		override.setName(environment.resolvePlaceholders("${" + PREFIX + ".name:${spring.application.name:application}}"));
		if (environment.containsProperty(PREFIX + ".profile")) {
			override.setProfile(environment.getProperty(PREFIX + ".profile"));
		}
		if (environment.containsProperty(PREFIX + ".label")) {
			override.setLabel(environment.getProperty(PREFIX + ".label"));
		}
		return override;
	}
	...
	public String toString() {
		return "ConfigClientProperties [enabled=" + this.enabled + ", profile=" + this.profile + ", name=" + this.name
				...
				+ Arrays.toString(this.uri) + ", discovery=" + this.discovery + ", failFast=" + this.failFast + ", token="
				+ this.token + ", requestReadTimeout=" + this.requestReadTimeout + ", sendState=" + this.sendState + ", headers="
				+ this.headers + "]";
	}
	...
	public static class Discovery {
		...
		/**
		 * Flag to indicate that config server discovery is enabled (config
