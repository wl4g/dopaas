 ...
 */
package com.wl4g.devops.uos.server;
...
import com.wl4g.devops.uos.server.config.ChannelServerProperties;
...
public class NettyCossServer extends CossServer {
	...
	protected void doStartBind() {
		...
		try {
			...
			f.addListener(future -> {
				...
				if (future.isSuccess()) {
					log.info("Netty uos server started on: {}/{}:{}", loopConfig.getChannelClass().getSimpleName(),
	...
	class CossServerHandler extends ChannelInitializer<SocketChannel> {
		...
		protected void initChannel(SocketChannel ch) throws Exception {
			log.info("Init uos netty channels...");
			...
			if (config.isEnableChannelLog()) {
				...
				log.info("Netty uos channel enable logLevel: {}", config.getChannelLogLevel());
			}
