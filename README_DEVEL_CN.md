...
│   ├── xcloud-dopaas-cmdb-service-facade # 逻辑层微服务spi接口，如 service接口类(主要为了让其他模块rpc交叉调用)
...
│   ├── xcloud-dopaas-uci-service-facade # 集成构建中心spi接口层(主要为了让其他模块rpc交叉调用)
...
│   ├── xcloud-dopaas-udc-service-facade # 开发中心spi接口层(主要为了让其他模块rpc交叉调用)
...
│   ├── xcloud-dopaas-udm-service-facade # 文档管理中心spi接口层(主要为了让其他模块rpc交叉调用)
...
│   ├── xcloud-dopaas-umc-service-facade # 监控中心管控服务spi接口层(主要为了让其他模块rpc交叉调用)
...
│   ├── xcloud-dopaas-urm-service-data # 仓库管理DAO接口层
│   ├── xcloud-dopaas-urm-service-facade # 仓库管理spi接口层(主要为了让其他模块rpc交叉调用)
│   ├── xcloud-dopaas-urm-service-starter-facade # 仓库管理spi接口实现层
│   └── xcloud-dopaas-urm-service-starter-manager # 仓库管理web接口层
