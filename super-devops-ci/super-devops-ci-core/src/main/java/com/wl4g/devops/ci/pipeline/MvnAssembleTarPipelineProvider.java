...
public class MvnAssembleTarPipelineProvider extends AbstractPipelineProvider {
    ...
    public void rollback() throws Exception {
        ...
        // Old file
        String oldFilePath = config.getBackup().getBaseDir() + "/" + getPipelineInfo().getAlias() + "/" + getPipelineInfo().getBranch() + "/" + subPackname(getPipelineInfo().getProject().getTarPath()) + "#"
    ...
    private void deploy() throws Exception {
        ...
        // backup in local
        backupLocal(getPipelineInfo().getPath() + getPipelineInfo().getProject().getTarPath(), getPipelineInfo().getTaskHistory().getId().toString(), getPipelineInfo().getAlias(), getPipelineInfo().getBranch());
        // scp to server
        List<Future<?>> futures = new ArrayList<>();
        for (AppInstance instance : getPipelineInfo().getInstances()) {
            // create deploy task
            Runnable task = new MvnAssembleTarPipelineHandler(this, getPipelineInfo().getProject(), getPipelineInfo().getPath(), instance, getPipelineInfo().getProject().getTarPath(),
