		...
		(function loadScripts(scripts, path, asyncLoad) {
			scripts.forEach(function(src, i) {
				...
				if (asyncLoad) { // 异步加载?
					...
					//loadObj.async = true;
					//loadObj.defer = true;
					if (document.body) { // (优先)尽量在body加载js
						document.body.appendChild(loadObj);
					} else {
						document.head.appendChild(loadObj);
					}
				} else {
					loadObj = document;
				...
				if (i == scripts.length - 1) {
					...
					loadObj.onload = loadObj.onreadystatechange = function(e) {
						if (!loadObj.readyState || loadObj.readyState == 'loaded' || loadObj.readyState == 'complete') {
