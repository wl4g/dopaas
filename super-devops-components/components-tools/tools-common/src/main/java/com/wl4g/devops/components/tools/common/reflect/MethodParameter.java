...
public class MethodParameter {
	 ...
	 * 
	 * @see #getNestingLevel()
	 * @deprecated since 5.2 in favor of retaining the original MethodParameter
	 *             and using {@link #nested(Integer)} if nesting is required
	 */
	@Deprecated
	 ...
	 * either in the form of Java 8's {@link java.util.Optional}, any variant of
	 * a parameter-level {@code Nullable} annotation (such as from JSR-305 or
	 * the FindBugs set of annotations), or a language-level nullable type
		...
		if (paramType == null) {
			if (this.parameterIndex >= 0) {
				...
				if (this.executable instanceof Constructor && ClassUtils2.isInnerClass(this.executable.getDeclaringClass())
					...
					// parameter
					// for inner classes with at least one generic constructor
					// parameter,
	...
	public Annotation[] getParameterAnnotations() {
		...
		if (paramAnns == null) {
			...
			if (this.executable instanceof Constructor && ClassUtils2.isInnerClass(this.executable.getDeclaringClass())
				...
				// Bug in javac in JDK <9: annotation array excludes enclosing
				// instance parameter
	...
	protected static int findParameterIndex(Parameter parameter) {
		...
		// Potentially try again with object equality checks in order to avoid
		// race
