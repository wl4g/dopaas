...
public class DefaultPipelineManager implements PipelineManager {
    ...
    protected void postPipelineRunSuccess(Integer taskId, PipelineProvider provider) {
        ...
        for (PipelineHistoryInstance pipelineHistoryInstance : pipelineHistoryInstances) {
            if (pipelineHistoryInstance.getStatus() != TASK_STATUS_SUCCESS && pipelineHistoryInstance.getStatus() != TASK_STATUS_CREATE) {
                allSuccess = false;
            } else {
                allFail = false;
            }
     ...
     * @return
     */
    protected PipelineProvider getPipelineProvider(PipelineHistory pipelineHistory, PipelineModel pipelineModel,ActionControl actionControl) {
