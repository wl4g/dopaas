...
import com.wl4g.dopaas.uci.config.TestingReportProperties;
...
public class TestingReportFSController {
	...
	@Autowired
	protected TestingReportProperties config;
	...
	public RespBase<?> upload(@RequestParam(value = "file") MultipartFile file) {
		...
		String path = config.getUploadPath() + "/" + fileName;
		saveFile(file, path);
		resp.setData(config.getDownloadUrl() + "/" + fileName);
		return resp;
	}
