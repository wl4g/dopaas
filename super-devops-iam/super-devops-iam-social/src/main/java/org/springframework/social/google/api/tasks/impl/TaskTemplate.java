...
public class TaskTemplate extends AbstractGoogleApiOperations implements TaskOperations {
	...
	@Override
	public Task createTaskAt(final String taskListId, final String parent, final String previous, final Task task) {
		isNull(task.getId(), "Task ID should not be null.");
		final StringBuilder sb = new StringBuilder(TASKS_URL).append(defaultIfBlank(taskListId, DEFAULT)).append(TASKS)
				.append('?');
		if (hasText(parent)) {
	...
	@Override
	public Task moveTask(final String taskListId, final Task task, final String parent, final String previous) {
