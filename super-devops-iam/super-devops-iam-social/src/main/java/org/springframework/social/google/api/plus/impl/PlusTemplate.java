...
public class PlusTemplate extends AbstractGoogleApiOperations implements PlusOperations {
	...
	public PlusTemplate(final RestTemplate restTemplate, final boolean isAuthorized) {
	...
	@Override
	public ActivitiesPage getActivities(final String userId, final String pageToken) {
		final StringBuilder sb = new StringBuilder(PEOPLE_URL).append(userId).append(ACTIVITIES_PUBLIC);
		if (pageToken != null) {
	...
	@Override
	public ActivitiesPage getActivities(final String userId) {
		return getActivities(userId, null);
	}

	@Override
	public ActivitiesPage searchPublicActivities(final String query, final String pageToken) {
	...
	@Override
	public ActivityComment getComment(final String id) {
		return getEntity(COMMENTS_URL + id, ActivityComment.class);
	}

	@Override
	public ActivityCommentsPage getComments(final String activityId, final String pageToken) {
		final StringBuilder sb = new StringBuilder(ACTIVITIES_URL).append(activityId).append(COMMENTS);
	...
	@Override
	public PeoplePage getPeopleInCircles(final String id, final String pageToken) {
		final StringBuilder sb = new StringBuilder(PEOPLE_URL).append(id).append("/people/visible");
		if (hasText(pageToken)) {
			sb.append("?pageToken=").append(pageToken);
		}
	...
	@Override
	public PeoplePage searchPeople(final String query, final String pageToken) {
		return personQuery().searchFor(query).fromPage(pageToken).getPage();
	}

	@Override
	public PeoplePage getActivityPlusOners(final String activityId, final String pageToken) {
		return getEntity(ACTIVITIES_URL + activityId + PLUSONERS, PeoplePage.class);
	}

	@Override
	public PeoplePage getActivityResharers(final String activityId, final String pageToken) {
		return getEntity(ACTIVITIES_URL + activityId + RESHARERS, PeoplePage.class);
	}

	@Override
	public Moment insertMoment(final Moment moment) {
	...
	@Override
	public MomentsPage getMoments(final String pageToken) {
