...
class TaskQueryBuilderImpl extends ApiQueryBuilderImpl<TaskQueryBuilder, TasksPage> implements TaskQueryBuilder {
	...
	TaskQueryBuilderImpl(final RestTemplate restTemplate) {
		super(TasksPage.class, restTemplate);
	}

	@Override
	public TaskQueryBuilder fromTaskList(final String taskListId) {
		this.taskListId = taskListId;
		return this;
	}

	@Override
	public TaskQueryBuilder completedFrom(final Date completedMin) {
		return appendQueryParam("completedMin", completedMin);
	}

	@Override
	public TaskQueryBuilder completedUntil(final Date completedMax) {
		return appendQueryParam("completedMax", completedMax);
	}

	@Override
	public TaskQueryBuilder dueFrom(final Date dueMin) {
		return appendQueryParam("dueMin", dueMin);
	}

	@Override
	public TaskQueryBuilder dueUntil(final Date dueMax) {
		return appendQueryParam("dueMax", dueMax);
	}

	@Override
	public TaskQueryBuilder updatedFrom(final Date updatedMin) {
		return appendQueryParam("updatedMin", updatedMin);
	}

	@Override
	public TaskQueryBuilder includeCompleted(final boolean showCompleted) {
		return appendQueryParam("showCompleted", showCompleted);
	}

	@Override
	public TaskQueryBuilder includeDeleted(final boolean showDeleted) {
		return appendQueryParam("showDeleted", showDeleted);
	}

	@Override
	public TaskQueryBuilder includeHidden(final boolean showHidden) {
