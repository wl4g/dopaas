...
public class DriveTemplate extends AbstractGoogleApiOperations implements DriveOperations {
	...
	public DriveFile copy(final String id, final String[] parentIds) {
		final DriveFile file = new DriveFile.Builder().setParents(parentIds).build();
	...
	public DriveFile copy(final String id, final String[] parentIds, final String title) {
		final DriveFile file = new DriveFile.Builder().setTitle(title).setParents(parentIds).build();
		return saveEntity(DRIVE_FILES_URL + id + "/copy", file);
	}
	...
	public DriveFile upload(final Resource resource, final DriveFile metadata, final UploadParameters parameters) {
		...
		final ResponseEntity<DriveFile> response = restTemplate.exchange(MULTIPART_UPLOAD_URL + parameters, POST, entity,
				DriveFile.class);
	...
	public DriveFile createFolder(final String parentId, final String name) {
		return createFileMetadata(
				DriveFile.builder().setMimeType(FOLDER).setTitle(name).setParents(hasText(parentId) ? parentId : "root").build());
	}
	...
	@Override
	public UserPermission addPermission(final String fileId, final UserPermission permission,
			final boolean sendNotificationEmails) {
		return saveEntity(DRIVE_FILES_URL + fileId + PERMISSIONS + SEND_NOTIFICATION + sendNotificationEmails, permission);
	}

	@Override
	public UserPermission updatesPermission(final String fileId, final String permissionId, final UserPermission permission) {
		final Object patch = new PatchBuilder().set("role", permission.getRole())
				.set("additionalRoles", permission.getAdditionalRoles()).getMap();
	...
	@Override
	public FileRevision updateRevision(final String fileId, final String revisionId, final FileRevision revision) {
		final Object patch = new PatchBuilder().set("pinned", revision.isPinned()).set("publishAuto", revision.isPublishAuto())
				.set("published", revision.isPublished()).set("publishedOutsideDomain", revision.isPublishedOutsideDomain())
				.getMap();
	...
	public FileCommentsPage getComments(final String fileId, final String pageToken) {
		return fileCommentQueryBuilder(fileId).fromPage(pageToken).getPage();
	}
	...
	@Override
	public FileComment updateComment(final String fileId, final String commentId, final FileComment comment) {
		final Object patch = new PatchBuilder().set("content", comment.getContent()).getMap();
	...
	@Override
	public CommentReply addReply(final String fileId, final String commentId, final CommentReply reply) {
		return saveEntity(DRIVE_FILES_URL + fileId + COMMENTS + commentId + REPLIES, reply);
	}

	@Override
	public CommentReply updateReply(final String fileId, final String commentId, final String replyId, final CommentReply reply) {
		final Object patch = new PatchBuilder().set("content", reply.getContent()).set("verb", reply.getVerb()).getMap();
	...
	public FileProperty updateProperty(final String fileId, final FileProperty property) {
		final Object patch = new PatchBuilder().set("value", property.getValue()).set("visibility", property.getVisibility())
				.getMap();
