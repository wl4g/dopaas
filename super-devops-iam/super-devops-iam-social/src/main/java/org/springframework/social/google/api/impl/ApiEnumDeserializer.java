...
/**
 * Jackson {@link JsonDeserializer} that converts a camel case string to capital
 * underscored string and finds the enum value with that name.
 * 
 * @author Gabriel Axel
 *
 * @param <T>
...
public abstract class ApiEnumDeserializer<T extends Enum<?>> extends JsonDeserializer<T> {
	...
	public T deserialize(final JsonParser jp, final DeserializationContext ctxt) throws IOException {
		...
		final T value = (T) Enum.valueOf((Class) type, sb.toString());
