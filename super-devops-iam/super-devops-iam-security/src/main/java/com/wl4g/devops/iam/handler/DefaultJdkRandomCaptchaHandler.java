 ...
 * @since
 */
public class DefaultJdkRandomCaptchaHandler extends AbstractCaptchaHandler {
	...
	public DefaultJdkRandomCaptchaHandler(IamProperties config, JedisCacheManager cacheManager) {
	...
	protected void out(HttpServletResponse response, String capText) throws IOException {
		...
		// Write the data out
		ImageIO.write(createImage(capText), "JPEG", out);
	}
	 ...
	 * @return
	 */
	private BufferedImage createImage(String capText) {
		...
		char[] chs = capText.toCharArray();
		for (int i = 0; i < chs.length; i++) {
			this.drowText(g, String.valueOf(chs[i]), (i + 1));
		}
		g.dispose();
		return image;
	}
