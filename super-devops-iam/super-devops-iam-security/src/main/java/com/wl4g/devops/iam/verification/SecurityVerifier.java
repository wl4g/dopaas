...
public abstract interface SecurityVerifier {
     ...
     *
     * @param assertion If assertion is enabled, an exception is thrown when the
     ...
     *
     * @param params  parameter DTO model
     * @param request HttpServletRequest
     * @param factors Safety limiting factor(e.g. Client remote IP and login name)
     ...
     * @param factors       Safety limiting factor(e.g. Client remote IP and login
     *                      user-name)
     * @param verifiedToken The token verified in the previous step. See:
     *                      {@link #verify(List, HttpServletRequest, boolean)}
     * @param required      Whether it is necessary to validation (ignore the retry failed
     *                      cumulative amount)
     * @throws VerificationException
     */
