...
f.set("colorDepth",b.components.get("colorDepth"));b=new Map;for(var g=Common.Util.toUrl({},f),f=100+parseInt(100*Math.random()),k=parseInt(f%3+1),u=0;u<k;u++)g=Common.Util.Codec.encodeBase58(g);g=f+"!"+g;console.debug("Generated apply umidToken data: "+g);b.set("umdata",g);l("post","{applyUmTokenUri}",b,function(b){Common.Util.checkEmpty("init.onPostUmidToken",c.init.onPostUmidToken)(b);var e=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);Common.Util.isEmpty(b)||b.code!=
...
throw Error("No such secure algoritm of: "+h.useSecureAlgorithmName);}},safeCheck:{checkGeneric:{secretKey:null},checkCaptcha:{enabled:!1,support:null,applyUri:null},checkSms:{enabled:!1,mobileNum:null,remainDelayMs:null}},clientSecretKey:{},applyModel:{primaryImg:null,applyToken:null,verifyType:null},verifiedModel:{verified:!0,verifiedToken:null},flags:{isCurrentlyApplying:!1,isVerifying:!1}},q={captchaLen:5,captchaDestroy:function(a){var b=Common.Util.checkEmpty("captcha.input",c.captcha.input),
e=Common.Util.checkEmpty("captcha.img",c.captcha.img);$(e).unbind("click");$(e).attr({src:"./static/images/ok.png"});$(b).attr("disabled",!0);$(b).css({cursor:"context-menu"});a&&($(b).val(""),$(b).css({display:"none"}),$(e).attr({src:""}),$(e).css({display:"none"}))},captchaRender:function(){d.flags.isCurrentlyApplying=!1;var a=$(Common.Util.checkEmpty("captcha.input",c.captcha.input)),b=Common.Util.checkEmpty("captcha.img",c.captcha.img);a.val("");$(b).click(function(){n(!0)});l("get",v(),new Map,
function(e){d.flags.isCurrentlyApplying=!1;d.applyModel=e.data.applyModel;$(a).css({display:"none",cursor:"text"});$(a).removeAttr("disabled");$(b).css({display:"none"});var g=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);Common.Util.isEmpty(e)||e.code!=g?($(b).attr("title",e.message),$(b).unbind("click"),setTimeout(function(){$(b).click(function(){n(!0)})},15E3)):$(b).attr("src",e.data.applyModel.primaryImg)},function(b,a,d){console.debug("Failed to apply captcha, "+d);
...
c.captcha.use),b=Common.Util.checkEmpty("captcha.registry",c.captcha.registry),e;for(e in b)if(e==a)return b[e];throw"Illegal verifier type for '"+e+"'";},registry:{VerifyWithSimpleGraph:q,VerifyWithGifGraph:q,VerifyWithJigsawGraph:{captchaDestroy:function(a){var b=Common.Util.checkEmpty("captcha.panel",c.captcha.panel);a&&$(b).css({display:"none"})},captchaRender:function(){d.flags.isCurrentlyApplying=!1;var a=Common.Util.checkEmpty("captcha.panel",c.captcha.panel);$(a).JigsawIamCaptcha({verifyDataKey:Common.Util.checkEmpty("definition.verifyDataKey",
...
c.definition.verifyAnalyzeUri)+"?"+Common.Util.toUrl({},a)},n=function(a){a?(a=encodeURIComponent(Common.Util.getEleValue("account.principalInput",c.account.principalInput,!1)),r(a,function(b,a,g){b.enabled&&!d.flags.isCurrentlyApplying&&Common.Util.checkEmpty("captcha.getVerifier",c.captcha.getVerifier)().captchaRender()})):Common.Util.checkEmpty("captcha.getVerifier",c.captcha.getVerifier)().captchaRender()},z=function(a,b){if("qrcodePanel"==b){var e=p.querySelector("#iam_qrcode_panel_iframe");
if(null==e||0>=e.length){e=Common.Util.checkEmpty("sns.qrcodePanel",c.sns.qrcodePanel);b=Common.Util.checkEmpty("qrcodePanel.src",e.src);var d=e.width||"250",f=e.height||"260",e=p.createElement("iframe");e.setAttribute("id","iam_qrcode_panel_iframe");e.setAttribute("frameborder","1");e.setAttribute("scrolling","no");e.setAttribute("width",d);e.setAttribute("height",f);e.setAttribute("style","border:solid 0;");b.appendChild(e)}setTimeout(function(){var b=p.querySelector("#iam_qrcode_panel_iframe");
-1==navigator.userAgent.indexOf("MSIE")?b.src=a:b.location=a},2)}else if("pagePanel"==b){b=Common.Util.checkEmpty("sns.pagePanel",c.sns.pagePanel);var t=m.open(a,m,"modal\x3d"+(b.modal||"yes")+",width\x3d"+(b.width||"800px")+",height\x3d"+(b.height||"500px")+",resizable\x3d"+(b.resizable||"no")+",left\x3d"+(b.left||"250px")+",top\x3d"+(b.top||"100px")),k=setInterval(function(){var b=m.document.getElementsByTagName("body")[0].getAttribute("refreshUrl");null!=t&&t.closed&&(clearInterval(k),Common.Util.isEmpty(b)||
(Common.Util.getRootWindow(m).location.href=b))},200)}else throw"Unsupported panelType, use 'qrcodePanel' or 'pagePanel'";},A=function(){if(c.sns.enable){var a=Common.Util.checkEmpty("sns.provider",c.sns.provider),b;for(b in a){var e=a[b],d=Common.Util.checkEmpty(b+".src",e.src),e=Common.Util.checkEmpty(b+".panelType",e.panelType);d.setAttribute("provider",b);d.setAttribute("panelType",e);d.onclick=function(b){var a=b.srcElement;b=a.getAttribute("provider");var a=a.getAttribute("panelType"),e;e=Common.Util.checkEmpty("sns.required",
...
c.sns.required.refreshUrl));e+="\x26"+Common.Util.checkEmpty("definition.principalKey",c.definition.principalKey)+"\x3d"+d;e+="\x26"+Common.Util.checkEmpty("definition.refreshUrlKey",c.definition.refreshUrlKey)+"\x3d"+f}"pagePanel"==a?e+="\x26agent\x3dy":"qrcodePanel"==a&&(e+="\x26agent\x3dn");if(!c.sns.onBefore(b,a,e))return console.warn("onBefore has blocked execution"),this;z(e,a)}}}else console.debug("SNS authenticator not enable!")},r=function(a,b){$(function(){if(Common.Util.checkEmpty("init.onPreCheck",
c.init.onPreCheck)(a)){var e=new Map;e.set("{principalKey}",a);e.set("{verifyTypeKey}",Common.Util.checkEmpty("captcha.use",c.captcha.use));e.set("{umidTokenKey}",d.umid.getValue());e.set("{secureAlgKey}",d.handshake.handleChooseSecureAlg());l("post","{checkUri}",e,function(a){Common.Util.checkEmpty("init.onPostCheck",c.init.onPostCheck)(a);var e=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);Common.Util.isEmpty(a)||a.code!=e||(d.safeCheck=$.extend(!0,d.safeCheck,a.data),
b(a.data.checkCaptcha,a.data.checkGeneric,a.data.checkSms))},function(a){console.log("Failed to safe check, "+a);Common.Util.checkEmpty("init.onError",c.init.onError)(a)},!0)}else console.warn("Skip the init safeCheck, because onPreCheck() return false")})},B=function(){c.captcha.enable?$(function(){n(!0);if("VerifyWithSimpleGraph"==c.captcha.use||"VerifyWithGifGraph"==c.captcha.use){var a=$(Common.Util.checkEmpty("captcha.input",c.captcha.input));a.attr("maxlength",Common.Util.checkEmpty("captcha.getVerifier",
c.captcha.getVerifier)().captchaLen);a.keyup(function(){if(d.safeCheck.checkCaptcha.enabled){var b=a.val();if(!Common.Util.isEmpty(b)&&b.length>=parseInt(a.attr("maxlength"))&&!d.flags.isVerifying){d.flags.isVerifying=!0;var e=new Map;e.put("{verifyDataKey}",b);e.put("{applyTokenKey}",Common.Util.checkEmpty("applyModel.applyToken",d.applyModel.applyToken));e.put("{verifyTypeKey}",Common.Util.checkEmpty("applyModel.verifyType",d.applyModel.verifyType));e.set("{umidTokenKey}",d.umid.getValue());l("post",
w(),e,function(a){d.flags.isVerifying=!1;var b=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);Common.Util.isEmpty(a)||a.code==b?(d.verifiedModel=a.data.verifiedModel,Common.Util.checkEmpty("captcha.getVerifier",c.captcha.getVerifier)().captchaDestroy(!1)):(n(!0),c.captcha.onError(a.message))},function(a){d.flags.isVerifying=!1;n(!0);c.captcha.onError(a)},!0)}}})}}):console.debug("Captcha verifier not enable!")},D=function(){c.account.enable?$(function(){$(p).bind("keydown",
function(a){13==a.keyCode&&$(Common.Util.checkEmpty("account.submitBtn",c.account.submitBtn)).click()});$(Common.Util.checkEmpty("account.submitBtn",c.account.submitBtn)).click(function(){var a=encodeURIComponent(Common.Util.getEleValue("account.principalInput",c.account.principalInput)),b=Common.Util.getEleValue("account.credentialInput",c.account.credentialInput);if(Common.Util.isAnyEmpty(a,b))c.account.onError(Common.Util.isZhCN()?"请输入账户名和密码":"Please input your account and password");else r(a,
function(e,g,f){d.clientSecretKey=IAMCrypto.RSA.generateKey();e=Common.Util.checkEmpty("Secret is empty",g.secretKey);e=encodeURIComponent(IAMCrypto.RSA.encryptToHexString(e,b));g="";if(d.safeCheck.checkCaptcha.enabled&&(g=d.verifiedModel.verifiedToken,Common.Util.isEmpty(g))){c.account.onError(Common.Util.isZhCN()?"请完成人机验证":"Please complete man-machine verify");n(!1);return}if(Common.Util.isAnyEmpty(a,e))c.account.onError("No empty login name or password allowed");else c.account.onBeforeSubmit(a,
e,g)&&($(Common.Util.checkEmpty("account.submitBtn",c.account.submitBtn)).attr("disabled",!0),f=new Map,f.set("{principalKey}",a),f.set("{credentialKey}",e),f.set("{clientSecretKey}",d.clientSecretKey.publicKeyHex),f.set("{clientRefKey}",C()),f.set("{verifiedTokenKey}",g),f.set("{verifyTypeKey}",Common.Util.checkEmpty("captcha.use",c.captcha.use)),f.set("{secureAlgKey}",d.handshake.handleChooseSecureAlg()),f.set("{umidTokenKey}",d.umid.getValue()),Common.Util.mergeMap(c.account.customParamMap,f),
l("post","{accountSubmitUri}",f,function(b){$(Common.Util.checkEmpty("account.submitBtn",c.account.submitBtn)).removeAttr("disabled");d.verifiedModel.verifiedToken="";var e=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);Common.Util.isEmpty(b)||b.code==e?($(p).unbind("keydown"),e=Common.Util.checkEmpty("Login successfully, response data.redirect_url is empty",b.data[c.definition.redirectUrlKey]),c.account.onSuccess(a,b.data)&&(Common.Util.getRootWindow(m).location.href=e)):
(n(!0),c.account.onError(b.message))},function(a){$(Common.Util.checkEmpty("account.submitBtn",c.account.submitBtn)).removeAttr("disabled");d.verifiedModel.verifiedToken="";c.account.onError(a)},!0))})})}):console.debug("Account authenticator not enable!")},E=function(){c.sms.enable?$(function(){$(Common.Util.checkEmpty("sms.sendSmsBtn",c.sms.sendSmsBtn)).click(function(){var a=Common.Util.getEleValue("sms.mobileArea",c.sms.mobileArea,!1)+Common.Util.getEleValue("sms.mobile",c.sms.mobile,!1);if(Common.Util.isEmpty(a))c.sms.onError("SMS login for mobile number is required.");
else{var b=$(Common.Util.checkEmpty("captcha.input",c.captcha.input)),f=b.val();if(d.safeCheck.captchaEnabled&&(Common.Util.isEmpty(f)||f.length<b.attr("maxlength")))c.account.onError("Illegal length of captcha input");else b=new Map,b.set("{principalKey}",encodeURIComponent(a)),b.set("{verifiedTokenKey}",f),l("post","{smsApplyUri}",b,function(a){var b=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);if(Common.Util.isEmpty(a)||a.code==b){c.sms.onSuccess(a);var e=parseInt(a.data.checkSms.remainDelayMs/
...
b)){var d=new Map;d.set("{principalKey}",encodeURIComponent(a));d.set("{credentialKey}",b);d.set("{smsActionKey}",Common.Util.checkEmpty("definition.smsActionValueLogin",c.definition.smsActionValueLogin));l("post","{smsSubmitUri}",d,function(a){var b=Common.Util.checkEmpty("definition.codeOkValue",c.definition.codeOkValue);if(Common.Util.isEmpty(a)||a.code==b)c.sms.onSuccess(a),Common.Util.getRootWindow(m).location.href=a.data.redirect_url;else c.sms.onError(a.message)},function(a){c.sms.onError(a)},
...
c);a=c;break}Common.Util.isEmpty(a)&&(a="Unknown",console.warn("Unknown platform browser ["+navigator.appVersion+"]"));return a},l=function(a,b,e,g,f,h){e.set("{responseType}",Common.Util.checkEmpty("definition.responseTypeValue",c.definition.responseTypeValue));h&&d.handshake.handleSessionTo(e);h=Common.Util.checkEmpty("deploy.baseUri",c.deploy.baseUri);b.startsWith("{")&&b.endsWith("}")&&(b=b.substr(1,b.length-2));h+=Common.Util.checkEmpty("definition",c.definition[b]);b=new Map;if("POST"==a.toUpperCase()||
"DELETE"==a.toUpperCase()){var k=d.that.getXsrfToken();b.set(k.headerName,k.value);k=d.that.generateReplayToken();b.set(k.headerName,k.value)}$.ajax({url:h,type:a,headers:JSON.fromMap(b),data:Common.Util.toUrl(c.definition,e),xhrFields:{withCredentials:!0},success:function(a,b,c){g(a)},error:function(a,b,c){f(c)}})},y=function(){return d.umid.getValuePromise().then(function(a){return d.handshake.getValuePromise(a)})};m.IAMCore=function(a){d.that=this;c=$.extend(!0,c,a);console.debug("After merge settings: "+
JSON.stringify(c));Common.Util.isEmpty(c.deploy.baseUri)&&(c.deploy.baseUri=x(),console.debug("Using overlay iamBaseURI: "+c.deploy.baseUri));sessionStorage.setItem(h.baseUriStoredKey,c.deploy.baseUri)};IAMCore.prototype.getUMToken=function(){return runtim.umid.getValue()};IAMCore.prototype.safeCheck=function(a,b){y().then(function(c){r(a,b)});return this};IAMCore.prototype.anyAuthenticators=function(){return this.accountAuthenticator().smsAuthenticator().snsAuthenticator().captchaVerifier()};IAMCore.prototype.accountAuthenticator=
function(){c.account.enable=!0;return this};IAMCore.prototype.smsAuthenticator=function(){c.sms.enable=!0;return this};IAMCore.prototype.snsAuthenticator=function(){c.sns.enable=!0;return this};IAMCore.prototype.captchaVerifier=function(){c.captcha.enable=!0;return this};IAMCore.prototype.build=function(){y().then(function(a){D();E();A();B()})};IAMCore.prototype.destroy=function(){sessionStorage.removeItem(h.baseUriStoredKey);sessionStorage.removeItem(h.umidTokenStorageKey);c=d=q=h=null;console.log("Destroyed IAMCore instance.")};
IAMCore.prototype.getXsrfToken=function(a){var b=Common.Util.checkEmpty("definition.xsrfTokenHeaderKey",c.definition.xsrfTokenHeaderKey),e=Common.Util.checkEmpty("definition.xsrfTokenParamKey",c.definition.xsrfTokenParamKey),d="IAM-"+location.hostname.split(".").slice(0,1).join(".").toUpperCase()+"-XSRF-TOKEN",d=a?a:d,f=Common.Util.getCookie(d,null);f||(a=IAMCore.getIamBaseUri()+Common.Util.checkEmpty("definition.applyXsrfTokenUrlKey",c.definition.applyXsrfTokenUrlKey),$.ajax({url:a,type:"GET",async:!1,
xhrFields:{withCredentials:!0},success:function(a,b,c){f=Common.Util.getCookie(d)},error:function(a,b,c){console.debug("Failed to init xsrf token. "+c)}}));return{headerName:b,paramName:e,value:f}};IAMCore.prototype.generateReplayToken=function(){for(var a=(new Date).getTime(),b="",e=0;2>e;e++)b+=Math.random().toString(36).substr(2);for(var e=Common.Util.sortWithAscii(b+a),d=Common.Util.Crc16CheckSum.crc16Modbus(e),d=parseInt(d%e.length/Math.PI)+1,f=e,e=0;e<d;e++)f=CryptoJS.MD5(f).toString(CryptoJS.enc.Hex);
e=JSON.stringify({n:b,t:a,s:f});a=Common.Util.checkEmpty("definition.replayTokenHeaderKey",c.definition.replayTokenHeaderKey);b=Common.Util.checkEmpty("definition.replayTokenParamKey",c.definition.replayTokenParamKey);d=Common.Util.Codec.encodeBase58(e);console.debug("Generated replay token(plain): "+e+" - "+d);return{headerName:a,paramName:b,value:d}};IAMCore.getIamBaseUri=function(){var a=sessionStorage.getItem(h.baseUriStoredKey);a||sessionStorage.setItem(h.baseUriStoredKey,a=x());return a}})(window,
document);