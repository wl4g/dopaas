...
r=Common.Util.getCookie(m,null);h.debug("Loaded cache xsrfTokenValue:",r,"by cookieName:",m);var k=!a;r||(h.debug("Loading new xsrf token..."),f=IAMCore.getIamBaseUri()+Common.Util.checkEmpty("definition.applyXsrfTokenUrlKey",c.definition.applyXsrfTokenUrlKey),Common.Util.Http.request({url:f,type:"HEAD",async:!k,xhrFields:{withCredentials:!0},success:function(c,f,e){r=Common.Util.getCookie(m);h.info("Loaded new xsrfTokenValue:",r,"by cookieName:",m);k||a(g(b,d,r))},error:function(a,b,c){h.error("Failed to init xsrf token. "+
c)}}));if(k)return g(b,d,r)},w=function(){for(var a=(new Date).getTime(),b="",d=0;2>d;d++)b+=Math.random().toString(36).substr(2);for(var d=Common.Util.sortWithAscii(b+a),g=Common.Util.Crc16CheckSum.crc16Modbus(d),g=parseInt(g%d.length/Math.PI)+1,f=d,d=0;d<g;d++)f=CryptoJS.MD5(f).toString(CryptoJS.enc.Hex);d=JSON.stringify({n:b,t:a,s:f});a=Common.Util.checkEmpty("definition.replayTokenHeaderKey",c.definition.replayTokenHeaderKey);b=Common.Util.checkEmpty("definition.replayTokenParamKey",c.definition.replayTokenParamKey);
