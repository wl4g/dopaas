...
public class StandardSecurityConfigurer implements ServerSecurityConfigurer {
    ...
    public IamPrincipalInfo getIamAccount(Parameter parameter) {
        ...
        if (nonNull(user)) {
            ...
            groups.forEach((identification) -> {
                identifications.add(identification.getIdentification());
            });
            PrincipalOrganization organization = new PrincipalOrganization();
            organization.setIdentifications(identifications);
            return new SimplePrincipalInfo(String.valueOf(user.getId()), user.getUserName(), user.getPassword(),
                    getRoles(user.getUserName()), getPermissions(user.getUserName())).setOrganization(organization);
        }
        return null;
    }
