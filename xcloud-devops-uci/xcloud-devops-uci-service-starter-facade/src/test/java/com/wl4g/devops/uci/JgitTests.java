...
public class JgitTests {
	...
	final private static UsernamePasswordCredentialsProvider usernamePasswordCredentialsProvider = new UsernamePasswordCredentialsProvider(
			"heweijie", "hwj13535248668");
	...
	@Test
	public void checkoutTest() throws IOException, GitAPIException {// use this,
																	// meger
																	// checkout
																	// branch
																	// and tag
		...
		if (exist) { // Exist to checkout
			...
			git.checkout().setCreateBranch(true).setName(branchName).setStartPoint("origin/" + branchName).setForceRefUpdate(true)
	...
	public void tagListTest() throws IOException, GitAPIException {
		Git git = Git.open(new File(gitPath));
		/*
		 * ListTagCommand listTagCommand = git.tagList(); List<Ref> call =
		 * listTagCommand.call(); for(Ref ref : call){
		 * System.out.println(ref.getName()); }
		 */
	...
	public void testFilter() {
		...
		list.add(5);
		list.add(6);
		List<Integer> instances = safeList(list).stream().map(detail -> detail + 1)
				.filter(instance -> instance == 2 || instance == 3).collect(toList());
