...
/**
 * OpenCv dynamic link library loader.
 * 
...
public final class OpenCvNativeLibraryLoader {
	...
	public final synchronized static void loadLibrarys() {
		if (loaded.compareAndSet(false, true)) {
			try {
				if (IS_OS_WINDOWS) {
					registerLibrarys0("libs/links/windows/" + getArchPath());
				} else {
					registerLibrarys0("libs/links/linux/" + getArchPath());
				}
			} catch (Exception e) {
				throw new IllegalStateException("Failed to register librarys", e);
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	private final synchronized static void registerLibrarys0(String basePath) throws Exception {
		URL baseUrl = OpenCvNativeLibraryLoader.class.getClassLoader().getResource(basePath);
		if (nonNull(baseUrl)) {
