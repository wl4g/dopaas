...
package com.wl4g.dopaas.uds.elasticjob.web.model;
...
public final class JobEventRdbSearch {
	...
	private static final List<String> FIELDS_JOB_EXECUTION_LOG = Arrays.asList("id", "hostname", "ip", "task_id", "job_name",
			"execution_source", "sharding_item", "start_time", "complete_time", "is_success", "failure_cause");

	private static final List<String> FIELDS_JOB_STATUS_TRACE_LOG = Arrays.asList("id", "job_name", "original_task_id", "task_id",
			"slave_id", "source", "execution_type", "sharding_item", "state", "message", "creation_time");
	...
	private List<JobExecutionEvent> getJobExecutionEvents(final Condition condition) {
		...
		try (Connection conn = dataSource.getConnection();
			...
			while (resultSet.next()) {
				JobExecutionEvent jobExecutionEvent = new JobExecutionEvent(resultSet.getString(1), resultSet.getString(2),
						resultSet.getString(3), resultSet.getString(4), resultSet.getString(5),
						JobExecutionEvent.ExecutionSource.valueOf(resultSet.getString(6)),
						Integer.parseInt(resultSet.getString(7)), new Date(resultSet.getTimestamp(8).getTime()),
			...
			// TODO log failure directly to output log, consider to be
	...
	private List<JobStatusTraceEvent> getJobStatusTraceEvents(final Condition condition) {
		...
		try (Connection conn = dataSource.getConnection();
				PreparedStatement preparedStatement = createDataPreparedStatement(conn, TABLE_JOB_STATUS_TRACE_LOG,
						FIELDS_JOB_STATUS_TRACE_LOG, condition);
				ResultSet resultSet = preparedStatement.executeQuery()) {
			while (resultSet.next()) {
				JobStatusTraceEvent jobStatusTraceEvent = new JobStatusTraceEvent(resultSet.getString(1), resultSet.getString(2),
						resultSet.getString(3), resultSet.getString(4), resultSet.getString(5),
						JobStatusTraceEvent.Source.valueOf(resultSet.getString(6)), resultSet.getString(7),
						resultSet.getString(8), JobStatusTraceEvent.State.valueOf(resultSet.getString(9)),
