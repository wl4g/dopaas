...
import com.wl4g.dopaas.uds.data.elasticjoblite.domain.DataSourceFactory;
...
import com.wl4g.dopaas.uds.service.elasticjoblite.domain.EventTraceDataSourceConfigurations;
...
public final class EventTraceDataSourceConfigurationServiceImpl
	...
	private EventTraceDataSourceConfiguration findActivatedDataSourceConfiguration(final GlobalConfiguration configs) {
		for (EventTraceDataSourceConfiguration each : configs.getEventTraceDataSourceConfigurations()
	...
	public void afterPropertiesSet() {
		for (EventTraceDataSourceConfiguration each : loadGlobal().getEventTraceDataSourceConfigurations()
				.getEventTraceDataSourceConfiguration()) {
			try {
				afterLoad(each);
