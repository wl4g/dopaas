...
import com.wl4g.dopaas.uds.extension.elasticjoblite.config.DynamicDSAutoConfiguration;
import com.wl4g.dopaas.uds.extension.elasticjoblite.domain.DataSourceFactory;
...
import com.wl4g.dopaas.uds.extension.elasticjoblite.exception.JdbcDriverNotFoundException;
...
import com.wl4g.dopaas.uds.extension.elasticjoblite.service.EventTraceDataSourceConfigService;
...
public final class EventTraceDataSourceConfigServiceImpl implements EventTraceDataSourceConfigService, InitializingBean {
    ...
    @Autowired
    private DynamicDSAutoConfiguration.DynamicDataSource dynamicDataSource;
    ...
    private EventTraceDataSourceConfig findActivatedDataSourceConfiguration(final GlobalConfiguration configs) {
        for (EventTraceDataSourceConfig each : configs.getEventTraceDataSourceConfigurations().getEventTraceDataSourceConfiguration()) {
            if (each.isActivated()) {
                return each;
            }
        }
    ...
    public void afterPropertiesSet() {
        for (EventTraceDataSourceConfig each : loadGlobal().getEventTraceDataSourceConfigurations().getEventTraceDataSourceConfiguration()) {
            try {
                afterLoad(each);
