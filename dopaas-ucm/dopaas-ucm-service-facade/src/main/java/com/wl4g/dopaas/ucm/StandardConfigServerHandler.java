...
public class StandardConfigServerHandler implements CentralConfigServerHandler {
     ...
     * @return
     */
    // private ReleaseContent convertReleasePropertySource(VersionContentBean
    // vc) {
    // String filename = vc.getNamespace().toLowerCase();
    // Assert.state(contains(filename, "."), String.format("Invalid namespace
    // filename for: %s", filename));
    //
    // // Resolve file content
    // String fileType = filename.substring(filename.lastIndexOf(".") +
    // 1).toLowerCase();
    // Map<String, Object> source = PropertySources.resolve(Type.of(fileType),
    // vc.getContent());
